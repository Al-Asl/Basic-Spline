...
[ExecuteInEditMode,AddComponentMenu("Path/Path Mover")]
public class PathMover : MonoBehaviour
{
    ...
    {
        if (path == null)
            throw new System.Exception("no path assigned!");
    }
    ...
    void Update()
    {
        ...
        if(fastMode)
        {
            ...
            var d1 = path[segmentIndex].GetD1(Mathf.Clamp01(t)).magnitude;
    ...
    {
        if (applyRotation)
            path[segmentIndex].GetSample(t).Transform(path.transform).Apply(transform);
        else
            transform.position = path.transform.TransformPoint(path[segmentIndex].GetPoint(t));
    }
