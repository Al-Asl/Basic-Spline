...
public class Segment
{
    ...
    public void Iterate(System.Action<Vector3> excute, int res)
    {
        float s = 1f / res;
        float ss = s * s;
        var a = curve.a * ss * s;
    ...
    /// </summary>
    //using forward differencing https://en.wikipedia.org/wiki/Finite_difference
    public void Iterate(System.Action<Sample> excuteSample, int res)
    {
        float s = 1f / res;
        float ss = s * s;
        var a = curve.a * ss * s;
        ...
        for (int i = 0; i < res + 1; i++)
        {
            ...
            excuteSample(GetSample(t, point, d1));
            point += d1;
            d1 += d2;
            d2 += d3;
        }
