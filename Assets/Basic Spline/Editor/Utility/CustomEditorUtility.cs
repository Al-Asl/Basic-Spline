...
/// consider using it if you don't need multi editing.
/// </summary>
public abstract class BaseSO<T> : IDisposable where T : Object
{
    ...
    public void Update()
    {
        ...
        {
            var field = fields[i];
            var value = CustomEditorUtility.GetValue(field.FieldType, serializedObject.FindProperty(field.Name));
    ...
    public void Apply()
    {
        for (int i = 0; i < fields.Count; i++)
        {
            ...
            CustomEditorUtility.SetValue(field.FieldType, value, serializedObject.FindProperty(field.Name));
        }
